...
export default function RightPanel(props: IRightPanelProps) {
  const {type, data, elementId, setDrawPanelData} = props

  const findCurrentId = (id: string) => {
  ...
  const findCurrentElementAndChangeData = (id: string, key: string, changeData: string) => {
    for(let item of data) {
      if(item.id === id) {
        item[key] = changeData
      }
    }
    return setDrawPanelData([...data])
  }
  ...
  const generateRightPanel = () => {
    if(type === RIGHT_PANEL_TYPE.NONE) {
      ...
      const elementData = findCurrentId(elementId)
